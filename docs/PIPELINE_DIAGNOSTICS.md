...
artifacts. The canonical product is `diagnostic-trace-v1.json`; Markdown, Mermaid, and
...
The stages are transcript acquisition, rule baseline, coarse discovery, candidate
selection, joining, fine refinement, and final disposition. Each stage records its input
and output ranges, boundary movement, duration added and removed, contract status,
warnings, and available evidence.
...
and explicit instrumentation gaps.
...
- `diagnostic-trace-v1.json`: durable machine-readable evidence, including a snapshot of
...
V1 reconstructs the selected pre-refinement boundary from persisted coarse support blocks,
and existing artifacts do not persist rejected join attempts. Each trace declares these
limitations and the instrumentation required to resolve them. That keeps missing evidence
visible instead of silently turning it into a causal claim.
