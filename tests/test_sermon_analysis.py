...
class SermonAnalysisTests(unittest.TestCase):
            ...
            "segments": [
                ...
                },
                {
                    ...
                    "text": (
                        ...
                        "Romans 8:1 is also clear. John 99:1 is invalid."
                    ),
                },
            ],
        }
    ...
    def test_measures_selected_sermon_and_persists_traceable_scripture_evidence(self) -> None:
        ...
        self.assertEqual(3, values["scripture_reference_mentions"])
        self.assertEqual(3, values["distinct_scripture_passages"])
        self.assertEqual(2, values["distinct_scripture_books"])
        self.assertEqual(["John", "Romans"], values["scripture_books"])
        ...
        self.assertEqual(
            {"John 3:16", "John 3:16-17", "Romans 8:1"},
            {
    ...
    def test_changed_source_or_analyzer_version_intentionally_creates_new_run(self) -> None:
        first = analyze_sermon(self.database, self.video)
        versioned = analyze_sermon(self.database, self.video, analyzer_version="2")
        self.payload["segments"][1]["text"] += " See Matthew 5:3."
        self._write_payload()
        changed = analyze_sermon(self.database, self.video, analyzer_version="2")
