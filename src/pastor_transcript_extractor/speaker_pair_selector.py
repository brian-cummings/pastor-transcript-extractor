...
def select_next_speaker_pair(
    ...
    for index, observation_a in enumerate(candidates):
        for observation_b in candidates[index + 1 :]:
            ...
            if pair_key in history.excluded_pairs:
                continue
    ...
    observation_a, observation_b, _ = min(
        ...
        key=lambda pair: _rank_pair(
            pair[0], pair[1], observation_use, disfavored, condition_counts
        ),
    )

    prior_a = int(observation_use.get(observation_a.input_fingerprint, 0))
    prior_b = int(observation_use.get(observation_b.input_fingerprint, 0))
...
def _rank_pair(
    ...
    use_a = int(observation_use.get(observation_a.input_fingerprint, 0))
    use_b = int(observation_use.get(observation_b.input_fingerprint, 0))
    used_count = int(use_a > 0) + int(use_b > 0)
    dates_differ = (
